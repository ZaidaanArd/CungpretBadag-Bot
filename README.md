...
| AI Chat | Tanya AI pake Llama 3 (Groq) gratis dan paham Bahasa Indonesia |
| Leveling | XP otomatis dari chat, level up, leaderboard server |
| AFK | Set status AFK, notif otomatis saat di-mention |
| Info | Info user, info server, avatar, help |
| Utility | Polling, countdown timer, kata bijak, echo, sapaan |
...
| `!stop` | - | Stop music dan clear queue |
...
| Command | Alias | Fungsi |
|---|---|---|
| `!rank [@user]` | `!level`, `!xp`, `!lvl` | Cek level dan XP (progress bar) |
...
- Groq API Key (Groq Console) untuk AI Chat

### Langkah-langkah

1. Clone repositori
```bash
git clone https://github.com/ZaidaanArd/CungpretBadag-Bot
cd CungpretBadag-Bot
```
...
| `sqlite3` | Database leveling dan AFK (built-in Python) |

## Database

Bot menggunakan SQLite (`data/levels.db`) untuk menyimpan:
- Data leveling dan XP (tabel levels)
- Data AFK (tabel afk)
